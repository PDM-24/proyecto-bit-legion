...
controller.onSaleToggleProduct = async (req, res, next) => {
    try{
        const {id} = req.params;
        const user = req.user;
        ...
        let _likes = user["onSaleProducts"] || [];
        const alreadyLike = _likes.findIndex(_i => _i.equals(id)) >= 0;
        ...
        user ["onSaleProducts"] = _likes;

        const newUser = await (await user.save())
            .populate("onSaleProducts", "nombre precio descripcion imagenes");
        return res.status(200).json(newUser);
...
controller.findOnSale = async (req, res, next) => {
    try{
        const user =
        await (req.user)
        .populate({
